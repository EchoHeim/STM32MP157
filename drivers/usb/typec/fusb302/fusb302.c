...
static struct fwnode_handle *fusb302_fwnode_get(struct device_node *nd)
{
    struct fwnode_handle *fwnode;

//    fwnode = device_get_named_child_node(dev, "connector");
    fwnode = fwnode_get_named_child_node(&nd->fwnode, "connector");
			 ...
			 const struct i2c_device_id *id)
{
    ...
    regmap_read(chip->regmap, 0x01, &tmp);

    if(tmp < 0x83 && tmp > 0x7F) {
    }
    else
        return -1;

    regmap_write(chip->regmap, 0x0c, 0x03);
    ...
    nd=of_find_node_by_path("/soc/i2c@40012000/stusb1600@28");

    chip->fwnode_connector = fusb302_fwnode_get(nd);
