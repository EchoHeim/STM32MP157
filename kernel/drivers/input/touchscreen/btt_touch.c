...
static int btt_touch_read(struct btt_touch_dev *btt_ts,
            u8 addr, u8 *buf, u16 len)
{
    struct i2c_client *client = btt_ts->client;
    ...
    if (2 == ret)
        ...
        dev_err(&client->dev, "%s: read error, ret=%d,addr=0x%x len=%d.\n",
...
static void btt_touch_poll(struct input_polled_dev *dev)
{
	...
	for (i = 0; i < regs.num_points; i++) {
        ...
        btt_touch_read(btt_ts, FT5422_TOUCH_DATA_REG + (6*i), &regs.point[i].xh, 4);

		x =RPI_TS_DEFAULT_WIDTH - ((((int)regs.point[i].xh & 0xf) << 8) + regs.point[i].xl);
		y =RPI_TS_DEFAULT_HEIGHT - ((((int)regs.point[i].yh & 0xf) << 8) + regs.point[i].yl);
            ...
            const struct i2c_device_id *id)
{
    ...
    ret = i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_WRITE_BYTE_DATA|
    ...
    btt_ts = devm_kzalloc(&client->dev, sizeof(struct btt_touch_dev), GFP_KERNEL);
    if (!btt_ts) {
        dev_err(&client->dev, "Failed to allocate btt touch driver data.\n");
        return -ENOMEM;
    }

    input_poll_dev = devm_input_allocate_polled_device(&client->dev);
    if (!input_poll_dev) {
        dev_err(&client->dev, "Failed to allocate btt touch polled driver data.\n");
        return -ENOMEM;
    }
    ...
    input_set_abs_params(input_poll_dev->input, ABS_MT_POSITION_X,
                0, RPI_TS_DEFAULT_WIDTH, 0, 0);
    input_set_abs_params(input_poll_dev->input, ABS_MT_POSITION_Y,
                0, RPI_TS_DEFAULT_HEIGHT, 0, 0);

    ret = input_mt_init_slots(input_poll_dev->input, MAX_SUPPORT_POINTS,INPUT_MT_DIRECT);
	if (ret) {
